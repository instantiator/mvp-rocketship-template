...
NB. By default, the authorizer requires the user's _identity token_. As soon as scopes are applied to API Gateway endpoints, it switches to requiring the _access token_.

* Instead of activating the first use / password reset process, AWS permits us to directly set the test user password.

Both methods work, but this is simpler.
...
- [ ] Add tests, and testing scripts for named stacks and transient stacks
- [ ] Apply OAuth scopes to API endpoints in `template.yaml`
